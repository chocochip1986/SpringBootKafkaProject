...
public class KafkaConsumerConfigs {
    ...
    public ConsumerFactory<String, byte[]> consumerByteFactory() {
        ...
        props.put(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG, 5000); //milliseconods to seconds => 5s
        return new DefaultKafkaConsumerFactory<>(props);
    }
...
//        factory.getContainerProperties().setConsumerRebalanceListener(new CustomRebalanceListener<String, byte[]>());
        return factory;
    }
    ...
    public RetryingBatchErrorHandler rbeh() {
        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer((KafkaOperations<String, byte[]>) kafkaByteTemplate,
                (r,e) -> new TopicPartition("topic.two.dlt", -1));
        return new RetryingBatchErrorHandler(new FixedBackOff(0L, 1L), recoverer);
    }
}
